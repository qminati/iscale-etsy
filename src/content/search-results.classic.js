...
const MAX_APPEARANCES = 30;

function searchResultKey(keyword, listingId) {
  return `search::${String(keyword || "").trim().toLowerCase()}::${listingId}`;
}

function globalRank(page, position) {
...
function currencyFromPrice(price) {
...
function parseTotalResultsDetail(doc) {
...
function parseTotalResults(doc) {
...
function detectSearchBlock(doc, href) {
...
function parseSearchResults(doc, href, nowIso) {
...
function mergeSearchResult(existing, incoming, nowIso) {
...
const SEARCH_EXPORT_COLUMNS = [
  ...
  api.searchResultKey = searchResultKey;
  api.globalRank = globalRank;
  api.currencyFromPrice = currencyFromPrice;
  api.parseTotalResultsDetail = parseTotalResultsDetail;
  api.parseTotalResults = parseTotalResults;
  api.detectSearchBlock = detectSearchBlock;
  api.parseSearchResults = parseSearchResults;
  api.mergeSearchResult = mergeSearchResult;
  api.MAX_APPEARANCES = MAX_APPEARANCES;
  api.SEARCH_EXPORT_COLUMNS = SEARCH_EXPORT_COLUMNS;
